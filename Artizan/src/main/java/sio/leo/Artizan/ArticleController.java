...
public class ArticleController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        TablePoste.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> showPersonDetails(newValue));
    }
...
@FXML
    private void handleEditPost() throws SQLException {
        Poste selectedPerson = TablePoste.getSelectionModel().getSelectedItem();
        if (selectedPerson != null) {
            boolean okClicked = App.showPersonEditDialog(selectedPerson);
            if (okClicked) {
                showPoste(selectedPerson);
